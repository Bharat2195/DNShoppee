...
public class UnusedPin extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		URL = "http://dnshoppee.com/mAndroid.aspx?MenuID=F&GID="+ preferences.getString(Login._name, "");
		XMLParser1 parser = new XMLParser1();
		String xml = parser.getXmlFromUrl(URL);
		Log.d("XML", xml);
		if (xml.trim().equalsIgnoreCase("No Data Found")) {
			Toast.makeText(UnusedPin.this, "No Data Found",
					Toast.LENGTH_LONG).show();
			showAlertDialog(context, "Warning", "Time Out, Try Again !!!",
					false);
			Log.d("XML", "DOCNo Data Found");
		} else if (xml.equals(" ")) {
			Toast.makeText(UnusedPin.this, "Time Out, Try Again !!!",
					Toast.LENGTH_LONG).show();
			...
			ReportClassfor4 adapter = new ReportClassfor4(
					UnusedPin.this, list1, list2, list3, list4);
