	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		_reset.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (curpsw.equals("")) {
				...
				} else if (curpsw.equals(preferences.getString(
						Login._pass, null)) == false) {
					showAlertDialog(context, "Worning", curpsw
							+ " is not Your Current Password ", false);
					_oldpass.setText("");
					...
					// }
					url = "http://dnshoppee.com/MAndroid.aspx?MenuID=C&oldpwd="+curpsw+"&newpwd=" +_confirm + "&GID="+preferences.getString(Login._name, "");
//					url = "http://globalappads.com/mglobalandroid.aspx?GID="
//							+ preferences.getString(Login._name, "")
//							+ "&MenuID=C&newpwd=" + _confirm + "&oldpwd="
//							+ curpsw;
	...
	private class GetResult extends AsyncTask<String, Void, String> {
		...
		protected void onPostExecute(String result) {
			...
			if (pDialog.isShowing()) {
				...
				if (line.trim()
						.equalsIgnoreCase("Password Change Successfully")) {
					Editor editor = preferences.edit();
					editor.putString(Login._pass, _confirm);
					editor.commit();
