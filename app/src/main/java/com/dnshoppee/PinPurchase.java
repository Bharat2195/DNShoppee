...
public class PinPurchase extends ActionBarActivity {

	EditText  noOfPinsE;
	String Pname, noOfPins;
	Spinner PnameE;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		purchase.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				} else if (noOfPins.equals("")) {
					Toast.makeText(getApplicationContext(),
							"Enter Number of Pin", Toast.LENGTH_SHORT).show();
				} else {

					String url = "http://dnshoppee.com/mANdroid.aspx?MenuId=O&GID="
							+ preferences.getString(Login._name, "")
							+ "&PackageName=" + Pname + "&NoofPins=" + noOfPins;
	...
	public class GetResult extends AsyncTask<String, Void, String> {
		...
		protected void onPreExecute() {
			// TODO Auto-generated method stub
			super.onPreExecute();
			_pDialog = new ProgressDialog(PinPurchase.this);
		...
		protected void onPostExecute(String result) {
			...
			if (_pDialog.isShowing()) {
				if (line.contains("Success")) {
					showAlertDialog1(_context, "Success", "Submitted",
							true);
				} else {
