...
public class Recharge1 extends Activity {
	...
	String[] DTH = new String[] { "AirtelDTH", "DishTV", "RelianceBigTV",
			"TataSky", "VideoconD2H", "SunDirect" };

	String[] POSTPAID = new String[] { "Airtel", "BSNL", "Docomo", "Idea",
			"Vodafone", };

	String[] PREPAID = new String[] { "Airtel", "Aircel", "BSNL", "Docomo",
			"Idea", "MTS", "Reliance", "TataIndicome", "Vodafone", "Uninor",
			"Videocon" };
	...
	String[] SPECIALR = new String[] { "BSNL STV", "DOCOMO STV", "Uninor STV",
			"Videocon STV" };
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (intent.hasExtra("plan")) {
			...
			if (rechargeType.equals("DataCard")) {
				...
				adapter = new ArrayAdapter<String>(context,
						android.R.layout.simple_spinner_item, DTH);
		...
		_showbal.setText(preferences.getString(Login._bal, "00.00"));
		// Column1
				...
				if (number.equals("")) {
					...
					if (flag_lanline) {
						...
						String url = "D R "
								...
								+ companyName + " " + rechargeType + " "
								+ number + " " + amount + " " + password;
						Log.i("com.dnsh", url);
						try {
							SmsManager smsManager = SmsManager.getDefault();
							smsManager.sendTextMessage(phno, null, url, null, null);
							Toast.makeText(getApplicationContext(), "please wait for response", Toast.LENGTH_SHORT).show();
